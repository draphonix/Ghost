...
const NavItem: React.FC<
    Omit<SettingNavItemProps, "isVisible" | "isCurrent"> & {
        keywords: string[];
    }
> = ({ keywords, navid, ...props }) => {
...
const Sidebar: React.FC = () => {
    const { filter, setFilter, checkVisible, noResult, setNoResult } =
        useSearch();
    ...
    // const [newslettersEnabled] = getSettingValues(settings, [
    ...
    const updateSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (e.target.value) {
            document
                .querySelector(".admin-x-settings")
    ...
    return (
        <div
            className="ml-auto flex w-full flex-col pt-0 tablet:max-w-[240px]"
            data-testid="sidebar"
        >
            ...
            <nav className={navClasses} id="admin-x-settings-sidebar">
                ...
                <SettingNavSection
                    isVisible={checkVisible(
                        Object.values(generalSearchKeywords).flat()
                    )}
                    title="Cài đặt chung"
                >
                    <NavItem
                    ...
                    <NavItem
                        icon="world-clock"
                        keywords={generalSearchKeywords.timeZone}
                        navid="timezone"
                        title="Múi giờ"
                        onClick={handleSectionClick}
                    />
                    ...
                    <NavItem
                        icon="layer"
                        keywords={generalSearchKeywords.metadata}
                        navid="metadata"
                        title="Meta data"
                        onClick={handleSectionClick}
                    />
                    <NavItem
                        icon="twitter-x"
                        keywords={generalSearchKeywords.twitter}
                        navid="twitter"
                        title="Thẻ X"
                        onClick={handleSectionClick}
                    />
                    <NavItem
                        icon="facebook"
                        keywords={generalSearchKeywords.facebook}
                        navid="facebook"
                        title="Thẻ Facebook"
                        onClick={handleSectionClick}
                    />
                    ...
                    <NavItem
                        icon="lock-locked"
                        keywords={generalSearchKeywords.lockSite}
                        navid="locksite"
                        title="Ẩn trang"
                        onClick={handleSectionClick}
                    />
                ...
                <SettingNavSection
                    isVisible={checkVisible(
                        Object.values(siteSearchKeywords).flat()
                    )}
                    title="Trang web"
                >
                    {/* <NavItem
                ...
                <SettingNavSection
                    isVisible={checkVisible(
                        Object.values(advancedSearchKeywords).flat()
                    )}
                    title="Nâng cao"
                >
                    <NavItem
                    ...
                    {/* <NavItem
                        icon="labs-flask"
                        keywords={advancedSearchKeywords.labs}
                        navid="labs"
                        title="Labs"
                        onClick={handleSectionClick}
                    /> */}
                    <NavItem
                        icon="time-back"
                        keywords={advancedSearchKeywords.history}
                        navid="history"
                        title="Lịch sử"
                        onClick={handleSectionClick}
                    />
                ...
                {!filter && (
                    <a
                        className="w-100 mb-10 mt-1 flex h-[38px] cursor-pointer items-center rounded-lg px-3 py-2 text-left text-[14px] font-medium text-grey-800 transition-all hover:bg-grey-200 focus:bg-grey-100 dark:text-grey-600 dark:hover:bg-grey-950 dark:focus:bg-grey-925"
                        onClick={() => {
                            updateRoute("about");
                        }}
                    >
                        <img
                            alt="Ghost Logo"
                            className="mr-[7px] h-[18px] w-[18px]"
                            src={GhostLogo}
                        />
                        About Ghost
                    </a>
