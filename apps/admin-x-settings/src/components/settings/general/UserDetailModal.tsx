...
import {
    RoutingModalProps,
    useRouting,
} from "@tryghost/admin-x-framework/routing";
import {
    User,
    canAccessSettings,
    hasAdminAccess,
...
const UserMenuTrigger = () => (
    <button
        className="flex h-8 cursor-pointer items-center justify-center rounded bg-[rgba(0,0,0,0.75)] px-3 opacity-80 hover:opacity-100"
        type="button"
    >
...
const UserDetailModalContent: React.FC<{ user: User }> = ({ user }) => {
    ...
    const {
        ...
        savedDelay: 500,
        onValidate: (values) => {
            return Object.entries(validators).reduce<ErrorMessages>(
                (newErrors, [key, validate]) => {
    ...
    const confirmSuspend = async (_user: User) => {
        if (
            ...
            _user.roles[0].name !== "Contributor"
        ) {
            try {
                ...
                if (error instanceof HostLimitError) {
                    NiceModal.show(LimitModal, {
                        ...
                        prompt:
                            error.message ||
                            `Your current plan doesn't support more users.`,
                        onOk: () =>
            ...
            prompt: (
                <>
                    <p className="mb-3">
                        ...
                        will be permanently deleted and all their posts will be
                        automatically assigned to the{" "}
                        <span className="font-bold">{owner.name}</span>.
                    </p>
                    <p>
                        To make these easy to find in the future, each post will
                        be given an internal tag of{" "}
    ...
    const coverEditButtonBaseClasses =
        "bg-[rgba(0,0,0,0.75)] rounded text-sm text-white flex items-center justify-center px-3 h-8 opacity-80 hover:opacity-100 transition-all cursor-pointer font-medium nowrap";

    const fileUploadButtonClasses = clsx(coverEditButtonBaseClasses);

    const deleteButtonClasses = clsx(coverEditButtonBaseClasses);

    const editButtonClasses = clsx(coverEditButtonBaseClasses);
            ...
            onOk={async () => {
                ...
                if (!(await handleSave({ fakeWhenUnchanged: true }))) {
                    showToast({
                        type: "pageError",
                        message:
                            "Can't save user, please double check that you've filled all mandatory fields.",
                    });
                }
            }}
        >
            <div>
                <div
                    className={`relative ${
                        canAccessSettings(currentUser)
                            ? "-mx-8 -mt-8 rounded-t"
                            : "-mx-10 -mt-10"
                    } bg-gradient-to-tr from-grey-900 to-black`}
                >
                    <div
                        className="flex min-h-[40vmin] flex-wrap items-end justify-between bg-cover bg-center"
                        style={{
                    ...
                    >
                        <div className="flex w-full max-w-[620px] flex-col gap-5 p-8 md:max-w-[auto] md:flex-row md:items-center">
                            ...
                            <div>
                                <Heading styles="break-words md:break-normal text-white">
                                    {user.name}
                                    {suspendedText}
                                </Heading>
                ...
                <div
                    className={`${
                        !canAccessSettings(currentUser) && "mx-auto max-w-4xl"
                    } mt-10 grid grid-cols-1 gap-x-12 gap-y-20 md:grid-cols-2`}
                >
                    <ProfileBasics
...
const UserDetailModal: React.FC<RoutingModalProps> = ({ params }) => {
    ...
    const user =
        currentUser.slug === params?.slug
            ? currentUser
            : users.find(({ slug }) => slug === params?.slug);
